...
public class BinarySearchTree {
	...
	private void add(BinaryNode node, BinaryNode leaf) {
		if(node.compareTo(leaf)<0) {
			if(leaf.getLeft()!=null) {
				add(node, leaf.getLeft());
			}
			leaf.setLeft(node);
			return;
		}
...
//		return tree;
	...
	private void inOrder(BinaryNode tree) {
		if (tree != null) {
	...
	private void preOrder(BinaryNode tree) {
		if (tree != null) {
			System.out.print(tree.getValue() + " | ");
	...
	private void postOrder(BinaryNode tree) {
		if (tree != null) {
	...
	private void revOrder(BinaryNode tree) {
		if (tree != null) {
	...
	private int getNumLevels(BinaryNode tree) {
		if (tree == null)
			return 0;
	...
	private int getNumLeaves(BinaryNode tree) {
		if (tree == null)
			return 1;
	...
	private int getWidth(BinaryNode tree) {
		if (tree == null)
			return 1;
	...
	private int getNumNodes(BinaryNode tree) {
		if (tree == null)
			return 0;
	...
	private boolean isFull(BinaryNode tree) {
	...
	private boolean search(BinaryNode tree, String c) {
		if (tree == null)
			return false;
	...
	private Comparable maxNode(BinaryNode tree, String c) {
		if (tree == null)
			return c;
	...
	private Comparable minNode(BinaryNode tree, String c) {
		if (tree == null)
			return c;
	...
	private void levelOrder(BinaryNode tree, Queue<Comparable> list) {
		if (tree == null)
			return;
	...
	private BinaryNode remove(Comparable val, BinaryNode tree) {
		if (tree != null) {
			...
			if (dirTest < 0)
				...
				if (tree.getRight() == null)
					...
					BinaryNode successor = getSmallest(tree.getRight());
	...
	private BinaryNode getSmallest(BinaryNode tree) {
		if (tree.getLeft() == null)
			return tree;
	...
	private String toString(BinaryNode tree, int level) {
		if (tree == null)
			return "";
